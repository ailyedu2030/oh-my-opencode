...
function buildDynamicSisyphusPrompt(
  ...
  // Load builtin commands and build command triggers section
  const builtinCommands = loadBuiltinCommands();
  const commandList = Object.values(builtinCommands).map(cmd => ({
    name: cmd.name,
    description: cmd.description ?? "",
    argumentHint: cmd.argumentHint,
  }));
  const commandTriggers = buildCommandTriggersSection(commandList);
