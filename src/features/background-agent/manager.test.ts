...
describe("BackgroundManager.handleEvent - session.error", () => {
  ...
  type RetryFallbackEntry = { providers: string[]; model: string; variant?: string }
  const createRetryTask = (manager: BackgroundManager, input: {
    id: string
    sessionID: string
    description: string
    concurrencyKey?: string
    fallbackChain?: RetryFallbackEntry[]
  }) => {
    const task = createMockTask({
      id: input.id,
  ...
  test("retry path releases current concurrency slot and prefers current provider in fallback entry", async () => {
    //#given - mock cache so anthropic is reachable for fallback (fallback-retry-handler imports from shared)
    const shared = require("../../shared")
    const connected = ["anthropic", "github-copilot", "opencode"]
    const connectedSpy = spyOn(shared, "readConnectedProvidersCache").mockReturnValue(connected)
    spyOn(shared, "readProviderModelsCache").mockReturnValue({
      connected,
      models: {},
  ...
  test("retry path triggers on session.status retry events", async () => {
    //#given - mock cache so anthropic is reachable for fallback
    const shared = require("../../shared")
    const connected = ["anthropic", "github-copilot", "opencode"]
    const connectedSpy = spyOn(shared, "readConnectedProvidersCache").mockReturnValue(connected)
    spyOn(shared, "readProviderModelsCache").mockReturnValue({
  ...
  test("retry path triggers on message.updated assistant error events", async () => {
    ...
    //#when - use type assertion for test payload (manager accepts broader event shape)
    manager.handleEvent({
    ...
    } as Parameters<BackgroundManager["handleEvent"]>[0])
