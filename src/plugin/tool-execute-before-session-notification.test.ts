const { describe, expect, test, spyOn, afterEach } = require("bun:test")
...
describe("createToolExecuteBeforeHandler session notification sessionID", () => {
  let getMainSessionIDSpy: { mockRestore?: () => void }
  ...
  test("uses main session fallback when input sessionID is empty", async () => {
    ...
    getMainSessionIDSpy = spyOn(sessionState, "getMainSessionID").mockReturnValue(mainSessionID)
