...
| **code-reviewer** | PR review, pre-landing review | Pre-landing PR review analyzing diffs for SQL safety, LLM trust boundaries, conditional side effects, and structural issues. |
...
The compaction system reduces context size while preserving critical information. Three layers work together:

### 3-Layer Architecture

| Layer | Purpose | Trigger |
|-------|---------|---------|
| **microCompact** | Remove stale tool results beyond gap threshold, keep recent messages | Every tool execution |
| **snipCompact** | Merge adjacent messages, remove empty content | Context window pressure |
| **autoCompact** | Full context analysis with model-aware token estimation | Hard limit approach |

### Token Estimation

Model-aware token counting with configuration:

```typescript
interface CompactConfig {
  modelContextLimits: Record<string, number>  // Model-specific limits
  defaultContextLimit: number                  // Default: 200,000
  autoThresholdBuffer: number                  // Trigger before hard limit
  reservedForSummary: number                  // Tokens reserved for summary
  minContentToKeep: number                    // Minimum content to preserve
}
```

### Preemptive Compaction

Automatically triggers compaction before hitting token limits:

- Uses cached token info from `message.updated` events
- Model-aware thresholds via `getDefaultCompactConfig()`
- Supports 1M context for models with extended context (`ANTHROPIC_1M_CONTEXT=true`)

## Tool Factory

Unified tool creation with permission pipeline:

### StreamingToolExecutor

Parallel and sequential tool execution with concurrency control:
...
// Parallel execution - results yielded as available
for await (const result of executor.execute(tools, context)) {
  console.log(result)
}
```
...
| Check | Purpose |
|-------|---------|
| **checkToolVisibility** | Is tool visible in current context? |
| **checkToolEnabled** | Is tool enabled in configuration? |
| **getToolPermission** | Permission gate (allow/deny/ask) |
...
Three isolation modes for subagent execution:

| Mode | Description |
|------|-------------|
| **copy** | Independent execution, no context sharing |
| **shared** | Context shared with parent session |
| **frozen** | Read-only, parent context frozen at fork |

### Worktree Manager

Git worktree-based isolation for subagent execution:

```typescript
const worktreeManager = new WorktreeManager(repoPath)
const worktree = await worktreeManager.createWorktree(agentId)
// Execute in isolated worktree
await worktreeManager.deleteWorktree(worktree.id)
```

### Subagent Registry

Tracks active subagent sessions with lifecycle management.
